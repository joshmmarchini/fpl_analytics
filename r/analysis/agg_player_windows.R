...
#---------------------------------------------------------------------
# 0 Define success:
# Prior window threshold: xgi_per_90_q3_prior
# Recent window threshold: xgi_per_90_q3_post
# Interpretation: Was this midfielder elite relative to peers in this window?
#---------------------------------------------------------------------

#---------------------------------------------------------------------
# Step 0: Define q3 prior & post xgi variables
#---------------------------------------------------------------------
...
mids_xgi <- mids_xgi %>%
  mutate(
    window = case_when(
      gameweek <= train_sample ~ "prior",
      gameweek >= test_sample ~ "recent",
      TRUE ~ NA_character_
    ),
    good_xgi = case_when(
      window == "prior" & xgi_per_90 >= xgi_per_90_q3_prior ~ 1,
      window == "recent" & xgi_per_90 >= xgi_per_90_q3_post ~ 1,
      TRUE ~ 0
    )
  )
...
#########################################
# 2) Bayesian Score for Defenders based on xgi
#########################################
...
# who was in the top 25% of xgi per 90?
def_prior <- top_performers %>%
  filter(window == "first_70%") %>%
  filter(position == "Defender") %>%
  filter(minutes_played >= min_threshold_train) # set earlier

# Determine q3 for xgi_per_90 in prior data
xgi_per_90_q3_prior_def <- quantile(
  def_prior$xgi_per_90,
  probs = 0.75,
  na.rm = TRUE
) # 0.14
